...
<el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="dialogStatus==='create'?createProduceData():updateProduceData()">
          {{ $t('table.confirm') }}
        </el-button>
      </div>
    </el-dialog>
...
<script>
import { getList,createData, updateData, searchData } from '@/api/producedata'
...
export default { 
  name:'TimingData',
  components: { Pagination , LineMarker },
  directives: { waves },
  data() {
    return {
      list:null,    
  ...
  methods: {
    ...
    deletefac(row){
      var vm=this;
     this.$axios.delete('https://localhost:44338/producedata?id='+row.id)
     .then(res=>{
    ...
    createProduceData() {
      createData(this.temp.year,this.temp.month,this.temp.date,this.temp.variety,this.temp.area,this.temp.count).then(res => {
        this.temp=res.data
    ...
    updateProduceData() {      
          updateData(this.temp.id,this.temp.variety,this.temp.area,this.temp.year,this.temp.month,this.temp.date,this.temp.count).then(() => {
            console.log(this.temp)
